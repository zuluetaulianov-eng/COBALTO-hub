...
async def test_irrelevant_query_behavior():
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        if 'avg_min_distance' in state.metrics:
            ...
            if avg_dist > 0.8:
                console.print("  [red]→ Very poor match (distance > 0.8)[/red]")
            elif avg_dist > 0.6:
                console.print("  [yellow]→ Poor match (distance > 0.6)[/yellow]")
            elif avg_dist > 0.4:
                console.print("  [blue]→ Moderate match (distance > 0.4)[/blue]")
            else:
                console.print("  [green]→ Good match (distance < 0.4)[/green]")
    ...
    async with AsyncWebCrawler() as crawler:
        ...
        try:
            ...
            if isinstance(state.coverage_shape, dict):
                console.print("Coverage model: centroid + radius")
...
async def main():
    ...
    try:
        ...
        try:
            ...
            console.print("[yellow]Warning: sentence-transformers not available[/yellow]")
