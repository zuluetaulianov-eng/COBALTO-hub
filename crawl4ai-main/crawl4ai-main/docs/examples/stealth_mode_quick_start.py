...
async def example_2_stealth_with_screenshot():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        if result.success:
            print("✓ Successfully crawled bot detection site")
            print("✓ With stealth enabled, many detection tests should show as passed")
            ...
            if result.screenshot:
                ...
                print("✓ Screenshot saved as 'stealth_detection_results.png'")
...
async def example_4_stealth_with_sessions():
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        ...
        print("✓ Session reused, maintaining cookies and state")
    ...
    async with AsyncWebCrawler(config=regular_config) as crawler:
        ...
        if result.success and result.screenshot:
            ...
            with open("comparison_without_stealth.png", "wb") as f:
                f.write(base64.b64decode(result.screenshot))
            print("  ✓ Screenshot saved: comparison_without_stealth.png")
            print("  Many tests will show as FAILED (red)")
    ...
    async with AsyncWebCrawler(config=stealth_config) as crawler:
        ...
        if result.success and result.screenshot:
            ...
            with open("comparison_with_stealth.png", "wb") as f:
                f.write(base64.b64decode(result.screenshot))
            print("  ✓ Screenshot saved: comparison_with_stealth.png")
            print("  More tests should show as PASSED (green)")
