# 🛰️ COBALTO HUB — Plataforma de Inteligencia OSINT C4I v14.4
...
> **v14.4** — Ingesta de fuentes públicas de Colombia (SECOP II, JEP, Rama Judicial, SIMCI/UNODC). Estandarización de ordenamiento cronológico por timestamp ISO. Conmutación dinámica de Ollama Local/Cloud. Suite de tests: 151/151 aprobados (100%).

---

## 🔄 Últimas Actualizaciones (Agosto 2026 — Release v14.4)

- **🇨🇴 Módulo de Extracción Teatro Colombia OSINT (`Teatro Colombia OSINT` / `osiris_colombia_recon.py`):** Arquitectura de extracción pública sin autenticación para fuentes oficiales colombianas: **SECOP II** (compras públicas vía API SODA Socrata `datos.gov.co`), **SIMCI / UNODC** (monitoreo de cultivos e incautaciones), **JEP** (Jurisdicción Especial para la Paz - Autos y Comunicados) y **Rama Judicial** (consulta de procesos y estrados judiciales). Exportado adicionalmente como paquete independiente de scraping/ingesta autónomo en la carpeta `Teatro Colombia OSINT/`.
- **⏱️ Estandarización de Ordenamiento Cronológico de Noticias (`dashboard_pipeline.py`, `app.py`, `intel-core.js`):** Estandarización completa de fechas de ingesta a formato **ISO 8601 UTC** (`published_iso`) y conversión a timestamps numéricos (`Date.parse()` en JS y `parse_datetime()` en Python). Se corrigió la discrepancia técnica donde noticias con texto RFC-822 (ej. `Wed, 21 Aug`) aparecían erróneamente en primer lugar debido al ordenamiento por comparación alfabética de texto plano (`localeCompare` / `str.sort`).
- **🧠 Inferencia Híbrida & Conmutación Dinámica de Ollama (`ollama_provider.py` & `intel-core.js`):** Sistema de gestión de sesiones HTTP `aiohttp` con clave de caché dinámica (`_sess_key`). Invalida y reconecta automáticamente la sesión al alternar entre endpoints local (`localhost:11434`) y remotos/nube sin reiniciar el backend. La consola RAG de la interfaz incluye redirección directa de errores al panel de configuración de IA.
- **🧪 Cobertura del 100% en Suite de Pruebas (`tests/`):** **151/151 tests pasados exitosamente** incluyendo endpoints `/api/intel/research`, recon de Colombia, resiliencia de extractores sociales (Bluesky/Mastodon/TwitterWebViewer), y autenticación de seguridad.
